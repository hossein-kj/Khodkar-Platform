...
$drpVersion.asDropdown("init"," First, select the code",{
    source: {
...
$drpPermission.asDropdown({
    source: {
        ...
        url: $.asInitService($.asUrls.cms_masterDataKeyValue_GetByOtherLanguagesAndTypeId, [{ name: '@typeId', value: 1032 },{ name: '@lang', value: $.asLang }])
        , displayDataField: 'Name'
        , valueDataField: 'MasterDataKeyValue.Id',
        orderby: 'MasterDataKeyValue.Order'
    }
...
$drpActions.asDropdown({
    source: {
        ...
        url: $.asInitService($.asUrls.cms_masterDataKeyValue_GetByOtherLanguagesAndTypeId, [{ name: '@typeId', value: 1031 },{ name: '@lang', value: $.asLang }])
        , displayDataField: 'Name'
          , valueDataField: 'MasterDataKeyValue.Id',
        orderby: 'MasterDataKeyValue.Order'
    }
...
var validateRule = {
    ...
    var getTypeMasterData = function(selectedId){
        ...
        $divMasterdata.asAfterTasks([
            loadData(url.replace("MasterDataKeyValue%2FTypeId%20eq%20@typeIdd","(" + query.join("%20or%20") + ")"))
          ...
          $drpLink.asDropdown("init","Choose Country",{
        source: {
            hierarchy:
            {
                type: 'flat',
...
var bindEvent = function () {
   ...
   $chkCache.change(function () {
        ...
        if(this.checked === true)
      {
            ...
            if(isLeaf === true)
         {
                 $.asShowMessage({ template: "error", message: "There is no way to add a permission to another one" })
  ...
  $btnDell.click(function () {
       if ($drpPermission.asDropdown('selected')) {
            ...
            $.asShowMessage({ template: "error", message: "To remove a permission, select." })
        }
  });
                             ...
                             $divVersion.asAfterTasks([
                                   loadData($.asInitService($.asUrls.develop_code_os_dotNet_getDotNetCodesVersionsById, [{ name: '@id', value: masterData.Id }]))
                                    ...
                                    $.each(versions,function(key,value){
                                     ...
                                     classes.push({Id:0,Value:"The last version that will be compiled or will be"});
