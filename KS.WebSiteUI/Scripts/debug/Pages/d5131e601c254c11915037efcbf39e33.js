...
$drpMasterData.asDropdown("init","Choose the type of Master Data",{
    source: {
        hierarchy:
        {
            type: 'flat',
...
$drpProtocolsPathOrUrl.asDropdown({
    source: {
        ...
        url: $.asInitService($.asUrls.cms_masterDataKeyValue_GetByOtherLanguagesAndTypeId, [{ name: '@typeId', value: 1046 },{ name: '@lang', value: $.asLang }])
        , displayDataField: 'Name'
          , valueDataField: 'MasterDataKeyValue.Id',
        orderby: 'MasterDataKeyValue.Order'
    }
...
var bindEvent = function () {
   ...
   $chkCache.change(function () {
     ...
     $chkType.change(function () {
        ...
        if(this.checked === true)
      {
         ...
         {
                 $.asShowMessage({ template: "error", message: "A MasterData can not be the type of masterdata" })
        ...
        if(this.checked === true)
      {
            ...
            if(isLeaf === true)
         {
                 $.asShowMessage({ template: "error", message: "There is no way to add a MasterData to another One" })
  ...
  $chkIsLeaf.change(function () {
        ...
        if(this.checked === true)
       {
         ...
         {
                 $.asShowMessage({ template: "error", message: "One type can not be a Masterdata" })
  ...
  $btnDell.click(function () {
       if ($drpMasterData.asDropdown('selected')) {
            ...
            $.asShowMessage({ template: "error", message: "To remove ,select a master data." })
        }
  });
    $btnSave.click(function () {
      ...
      $drpMasterDataType.on("change", function (event, item) {
        var url = $.asUrls.cms_masterDataKeyValue_GetByOtherLanguagesAndTypeId
        ...
        $drpMasterData.asDropdown("reload",{url: url.replace("MasterDataKeyValue%2FTypeId%20eq%20@typeIdd","(" + query.join("%20or%20") + ")").replace("@lang",$.asLang)});
