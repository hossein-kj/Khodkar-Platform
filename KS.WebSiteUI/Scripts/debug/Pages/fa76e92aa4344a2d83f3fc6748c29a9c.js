$('#ifa76e92aa4344a2d83f3fc6748c29a9c').on($.asEvent.page.loaded, function (event, requestedUrl, asPageParams) {
    ...
    $editorContent.asEditor({
        ...
        init_instance_callback: function (editor) {
            ...
            editor.on('focus', function (e) {
                changeEditorToolbar("Content")
            });
        }
    });
    var loadMenu = function () {
        return $.asAjaxTask({
            url: $.asInitService($.asUrls.cms_link_getByLanguage, [{ name: '@lang', value: $.asLang }])
        });
    }
    ...
    $drpService.asDropdown({
        source: {
            hierarchy:
            {
                ...
                childrenDataField: 'Children'
                , isLeafDataField: "MasterDataKeyValue.IsLeaf"
             , removeChildLessParent: true
            },
    ...
    $drpType.asDropdown({
        source: {
            ...
            url: $.asInitService($.asUrls.cms_masterDataKeyValue_GetByOtherLanguagesAndTypeId, [{ name: '@typeId', value: 1003 }, { name: '@lang', value: $.asLang }])
    ...
    $drpFrameWork.asDropdown({
        source: {
            ...
            url: $.asInitService($.asUrls.cms_masterDataKeyValue_GetByOtherLanguagesAndTypeId, [{ name: '@typeId', value: 1005 }, { name: '@lang', value: $.asLang }])
    ...
    var getwebform = function (formUrl, pageTypeId) {
        ...
        $frmCmsWebForm.asAjax({
            ...
            success: function (webForm) {
                ...
                //    setMode = true;
                if (webForm.Id === 0) {
                    $.asShowMessage({ message: 'The Content of This Menu Not Exist' });
    ...
    var bindEvent = function () {
        ...
        var find = function () {
            currentEditor.asCodeEditor("find", $txtFind.val(), {
                backwards: false,
                wrap: false,
        ...
        $codeEditorJavascript.asCodeEditor("editor").on("focus", function () {
            ...
            changeEditorToolbar("Java Script")
        });
        ...
        $(asPageEvent).on("compare", function (event, selectedId) {
            ...
            } else if ($lblEditor.html() === "Java Script") {
        ...
        $drpService.on("change", function (event, item) {
            //var result = $.grep(services, function (e) { return e.id == item.value; });
            //if (result.length != 1) return;
            //if (item.selected) {
            //$codeEditorJavascript.asCodeEditor("insert", result[0].code);
            //} else {
            //    $codeEditorJavascript.asCodeEditor("find", result[0].code);
