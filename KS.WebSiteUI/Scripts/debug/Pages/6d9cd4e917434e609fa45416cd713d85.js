...
$drpCodes.asDropdown({
    source: {
        ...
        ,url:$.asInitService($.asUrls.cms_masterDataKeyValue_GetByOtherLanguagesAndTypeId, [{ name: '@typeId', value: 1040 },{ name: '@lang', value: $.asLang }])
        , displayDataField: 'Name'
        ...
        var executeNoneQuery = function(){
         if(selectedConnectionId===0){
           ...
           $divEditor.asAjax({
            ...
            success: function (result) {
                as("#divResult").empty("")
                as("#divResult").html("Affected Row : "+result[0]);
                  $btnExec.button('reset'); 
              $.asShowMessage({ message: $.asRes[$.asLang].successOpration,showTime:10000000  });
            },
            error:function(){
         ...
         $frm.asAjax({
            ...
            success: function (result) {
              ...
              $drpCodes.asDropdown("removeItem");
                 ...
                 $.asShowMessage({ template: "error", message: "To remove, you must select a code" })
        }
    });
