...
var bindEvent = function () {
        ...
        if(this.checked === true)
      {
            ...
            if(isLeaf === true)
         {
                 $.asShowMessage({ template: "error", message: "There is no way to add a group to another one" })
       ...
       if ($drpGroups.asDropdown('selected')) {
            ...
            success: function (masterData) {
                // $drpGroups.asDropdown("removeItem");
                  $drpGroups.asDropdown("reload",{url: $.asInitService($.asUrls.security_Group_getAllByOtherLanguage, [{ name: '@lang', value: $.asLang }])});
              $.asShowMessage({ message: $.asRes[$.asLang].successOpration ,showTime:10000000 });
            }
        }, { $form: $frm });
       }else{
            $.asShowMessage({ template: "error", message: "To remove , select a group" })
        }
  });
