 ...
 $(selector).calendarsPicker({minDate: 0, maxDate: '+1m +1w'}) */
	...
	alert('Now in ' + month + '/' + year);
 	...
 	alert('Selected ' + dates);
		...
		_translateNumberFunction: function (num, inst) {
		   if(inst.options.translateNumberFunction == null)
		       return num;
		   else {
		       return inst.options.translateNumberFunction(num);
		   }
	    },
		_instSettings: function(elem, options) {
			...
			for (var week = 0; week < numWeeks; week++) {
				...
				for (var day = 0; day < calendar.daysInWeek(); day++) {
					...
					days += this._prepare(renderer.day, inst).replace(/\{day\}/g,
						...
						dateInfo.content || this._translateNumberFunction(drawDate.day(),inst) : '&nbsp;') +
						(selectable ? '</a>' : '</span>'));
					drawDate.add(1, 'd');
					jd++;
				}
		...
		_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader, calendar) {
			...
			if (yearRange === 'any') {
				selector = '<select class="' + this._monthYearClass + ' ' + this._anyYearClass +
					...
					'<option>' + this._translateNumberFunction(year,inst) + '</option></select>' +
				...
				var addYear = function(y,parent) {
					if (y !== 0 || calendar.hasYearZero) {
						selector += '<option value="' +
							Math.min(month, calendar.monthsInYear(y) - 1 + calendar.minMonth) +
							'/' + y + '"' + (year === y ? ' selected="selected"' : '') + '>' +
							parent._translateNumberFunction(y, inst) + '</option>';
					}
				};
...
/* http://keith-wood.name/calendars.html
   Farsi/Persian localisation for Persian calendar for jQuery v2.0.0.
