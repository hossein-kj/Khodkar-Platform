...
; (function ($, window, undefined) {
    ...
    var namespace = ".rs.jquery.asBootGrid";
    var source, dataAdepter, page
    ...
    function appendRow(row) {
        ...
        function exists(item) {
            ...
            return that.identifier && $.asGetPropertybyName(item,that.identifier) === $.asGetPropertybyName(row,that.identifier);
        }
    ...
    */
    ...
    function loadData(isSerach) {
        ...
        function containsPhrase(row) {
            ...
            for (var i = 0; i < that.columns.length; i++) {
                ...
                if (column.converter.to(row[column.id]) != null ) {
                    if (column.searchable && column.visible && (column.converter.to(row[column.id]) + "").search(searchPattern) > -1) { return true; }
        ...
        var loadComplete = function (data) {
            ...
            data = that.options.responseHandler(data);

            that.current = that.newCurrent;
            that.beforeSearchCurrent = that.newCurrent
            that.rows = data.rows
            that.beforeSearchTotal = data.total
            update(data.rows, data.total);

        }
        source = $.extend({ datatype: "json", loadComplete: loadComplete, page: page }, this.options.source);
        ...
        dataAdepter = $.extend({
            ...
            }, extraSettings: { loadingText: $.aswaitingViewSmall, overlayClass: 'as-overlay-absolute' }
        }, this.options.dataAdepter)
        ...
        //}
        if ((this.options.ajax || source.localData) && typeof (isSerach) === "undefined") {
            var request = getRequest.call(this)
            if (request !== null) {
                that.newCurrent = request.current
                source.url = request.url
                $.as(page).asDataAdepter(source, dataAdepter)
            } else {
                this.element._bgBusyAria(false)
            }

        }
        else {
            var total, rows = (this.searchPhrase.length > 0) ? this.rows.where(containsPhrase) : this.rows
            ...
            if (this.searchPhrase.length > 0)
                ...
                total = that.beforeSearchTotal
                this.current = that.beforeSearchCurrent
            }
    ...
    function renderActions() {
        if (this.options.navigation !== 0) {
            ...
            if (actionItems.length > 0) {
                ...
                // Refresh Button
                if (this.options.ajax || (this.options.source ? this.options.source.url !== "undefined":false)) {
    ...
    function renderRows(rows, append) {
        if (rows.length > 0) {
            ...
            $.each(rows, function (index, row) {
                var cells = "",
                    // rowAttr = " data-row-id=\"" + ((that.identifier == null) ? index : row[that.identifier]) + "\"",
                     rowAttr = " data-row-id=\"" + ((that.identifier == null) ? index : $.asGetPropertybyName(row,that.identifier)) + "\"",
                    rowCss = "";

                if (that.selection) {
                    // var selected = ($.inArray(row[that.identifier], that.selectedRows) !== -1),
                     var selected = ($.inArray($.asGetPropertybyName(row,that.identifier), that.selectedRows) !== -1),
                        selectBox = tpl.select.resolve(getParams.call(that,
                            // { type: "checkbox", value: row[that.identifier], checked: selected }));
                             { type: "checkbox", value: $.asGetPropertybyName(row,that.identifier), checked: selected }));
            ...
            if (typeof (append) != "undefined")
                $("#" + this.element._bgId() + " tr:last").after(html);
            else
                tbody.html(html);
            ...
            .on("click" + namespace, "> tr", function (e) {
                ...
                var $this = $(this),
                    ...
                    row = (that.identifier == null) ? that.currentRows[id] :
                        ...
                        that.currentRows.first(function (item) { return $.asGetPropertybyName(item,that.identifier) === id; });
    ...
    /**
     * Represents the jQuery asBootGrid plugin.
     *
     * @class Grid
     * @constructor
    ...
    var Grid = function (element, options) {
        ...
        this.sortDictionary = {};
        this.total = 0;
        this.beforeSearchTotal = 0;
        this.beforeSearchCurrent = 0;
        this.totalPages = 0;
        this.cachedParams = {
     ...
     * @example
     *   // Global approach
     *   $.asBootGrid.defaults.selection = true;
     * @example
     *   // Initialization approach
     *   $("#asBootGrid").asBootGrid({ selection = true });
     **/
    Grid.defaults = {
        ...
        responseHandler: function (response) {
            ...
            if ($.isArray(response)) {
                newResponse.rows = response
                newResponse.total = response.length
            } else {
         ...
         **/
        labels: {
            all: $.asRes[$.asLang].bootGrid.all,
            infos: $.asRes[$.asLang].bootGrid.infos,
            loading: $.asRes[$.asLang].bootGrid.loading,
            noResults: $.asRes[$.asLang].bootGrid.noResults,
            refresh: $.asRes[$.asLang].bootGrid.refresh,
            search: $.asRes[$.asLang].bootGrid.search
        },
    ...
    Grid.prototype.append = function (rows) {
        if (this.options.ajax) {
            ...
            //$("#" + this.element._bgId()).find('tr').hide();
            renderRows.call(this, appendedRows, true);
            ...
            if (this.options.ajax) {
                ...
                for (var i = 0; i < rowIds.length; i++) {
                    ...
                    for (var j = 0; j < this.rows.length; j++) {
                        ...
                        if ($.asGetPropertybyName(this.rows[j],this.identifier) === id) {
                ...
                $("#" + this.element._bgId()).find('.active').hide();
        ...
        if (this.selection) {
                ...
                if ($.inArray(id, this.selectedRows) === -1) {
                    for (i = 0; i < this.currentRows.length; i++) {
                        // if (this.currentRows[i][this.identifier] === id) {
                         if ($.asGetPropertybyName(this.currentRows[i],this.identifier) === id) {
            ...
            if (selectedRows.length > 0) {
                ...
                while (!this.options.keepSelection && selectMultiSelectBox && i < this.currentRows.length) {
                    ...
                    selectMultiSelectBox = ($.inArray($.asGetPropertybyName(this.currentRows[i++],this.identifier), this.selectedRows) !== -1);
                }
        ...
        if (this.selection) {
            ...
            if (deselectedRows.length > 0) {
                ...
                for (i = 0; i < deselectedRows.length; i++) {
                    ...
                    this.element.find("tbody > tr[data-row-id=\"" + $.asGetPropertybyName(deselectedRows[i],this.identifier) + "\"]")
    ...
    if (!Array.prototype.propValues) {
        Array.prototype.propValues = function (propName) {
            ...
            for (var i = 0; i < this.length; i++) {
                ...
                result.push($.asGetPropertybyName(this[i],propName));
            }
            return result;
        };
    }
    ...
    var old = $.fn.asBootGrid;

    $.fn.asBootGrid = function (option) {
        page = this
    ...
    $.fn.asBootGrid.Constructor = Grid;

    // GRID NO CONFLICT
    // ===============

    $.fn.asBootGrid.noConflict = function () {
        $.fn.asBootGrid = old;
        return this;
    };
    ...
    $("[data-toggle=\"asBootGrid\"]").asBootGrid();
})(jQuery, window);
/*! 
 * jQuery Bootgrid v1.3.1 - 09/11/2015
 * Copyright (c) 2014-2015 Rafael Staib (http://www.jquery-bootgrid.com)
...
; (function ($, window, undefined) {
    ...
    $.extend($.fn.asBootGrid.Constructor.defaults.css, {
        icon: "icon glyphicon",
        iconColumns: "glyphicon-th-list",
